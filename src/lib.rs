...
pub mod pallet {
	...
	// Config trait 用来配置pallet所必须的
	#[pallet::config]
	pub trait Config: frame_system::Config {
		// 存证（哈希值）长度限制
		#[pallet::constant]
		type MaxClaimLength: Get<u32>; // BoundedVec长度受限，接收u32长度的类型
	...
	// runtime不能使用vector类型，只能用BoundedVec
	#[pallet::storage]
	...
	impl<T: Config> Pallet<T> {
		...
		// allow the owner of a claim to transfer the claim
		#[pallet::weight(0)]
		pub fn transfer_claim(
			...
			Proofs::<T>::mutate(
				&bounded_claim,
				|val| {*val = Some((receiver.clone(), block_number))}
			);
